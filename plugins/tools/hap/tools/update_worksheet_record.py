    ...
    def _invoke(
        ...
        try:
            ...
            if res_json.get("error_code") != 1:
                yield self.create_text_message(f"Failed to update the record. {res_json['error_msg']}")
            yield self.create_text_message("Record updated successfully.")
        except httpx.RequestError as e:
            yield self.create_text_message(f"Failed to update the record, request error: {e}")
        except json.JSONDecodeError as e:
            yield self.create_text_message(f"Failed to parse JSON response: {e}")
        except Exception as e:
            yield self.create_text_message(f"Failed to update the record, unexpected error: {e}")
