...
class ListWorksheetRecordsTool(Tool):
    def _invoke(
        ...
        try:
            ...
            if res.is_success:
                if res_json["error_code"] != 1:
                    yield self.create_text_message("Failed to get the records. {}".format(res_json["error_msg"]))
                else:
                    result = {
                        "fields": fields,
                        "rows": [],
                    ...
                    if result_type == "json":
                        ...
                        if result["total"] > 0:
                            result_text += f" The following are {min(limit, result['total'])} pieces of data presented in a table format:\n\n{table_header}"
                            for row in rows:
                                result_values = []
                                for f in fields:
            ...
            yield self.create_text_message("Failed to get the records, something went wrong: {}".format(e))
    ...
    def handle_value_type(self, value, field):
        ...
        yield self.rich_text_to_plain_text(value) if value else ""
    ...
    def rich_text_to_plain_text(self, rich_text):
        text = re.sub("<[^>]+>", "", rich_text) if "<" in rich_text else rich_text
