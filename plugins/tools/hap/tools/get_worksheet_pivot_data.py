...
class GetWorksheetPivotDataTool(Tool):
    def _invoke(
        ...
        if not x_column_fields or not x_column_fields.startswith("["):
            yield self.create_text_message("Invalid parameter Column Fields")
        y_row_fields = tool_parameters.get("y_row_fields", "")
        if y_row_fields and (not y_row_fields.strip().startswith("[")):
            yield self.create_text_message("Invalid parameter Row Fields")
        ...
        if not value_fields or not value_fields.strip().startswith("["):
            yield self.create_text_message("Invalid parameter Value Fields")
        ...
        try:
            ...
            if res_json.get("status") != 1:
                yield self.create_text_message(f"Failed to get the worksheet pivot data. {res_json['msg']}")
            ...
            yield self.create_text_message(f"Failed to get the worksheet pivot data, request error: {e}")
        except json.JSONDecodeError as e:
            yield self.create_text_message(f"Failed to parse JSON response: {e}")
        except Exception as e:
            yield self.create_text_message(f"Failed to get the worksheet pivot data, unexpected error: {e}")
