...
class GetWorksheetFieldsTool(Tool):
    def _invoke(
        ...
        try:
            ...
            if res_json.get("error_code") != 1:
                yield self.create_text_message(f"Failed to get the worksheet information. {res_json['error_msg']}")
            ...
            yield self.create_text_message(f"Failed to get the worksheet information, request error: {e}")
        except json.JSONDecodeError as e:
            yield self.create_text_message(f"Failed to parse JSON response: {e}")
        except Exception as e:
            yield self.create_text_message(f"Failed to get the worksheet information, unexpected error: {e}")
    ...
    def get_controls(self, controls: list) -> dict:
        ...
        for control in controls:
            ...
            fields.append(field)
            fields_list.append(
                f"|{field['id']}|{field['name']}|{field['type']}|{field['typeId']}|{field['description']}|{field['options'] or ''}|"
            )
        fields.append(
            {
                "id": "ctime",
