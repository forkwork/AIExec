...
- Remove the "Replace Markdown Image Path" tool. Now, the image paths in the Markdown will be automatically replaced with previewable URLs (the validity period of the URL is determined by the FILES_ACCESS_TIMEOUT in aiexec.env).If you want to use this feature, please update the Aiexec's core code.![](./_assets/mineru2.jpg)
...
**You can download the YAML file and import it into Aiexec. This demo includes a basic capability demonstration of the plugin.**

[demo_dsl.yml](@plugins/tools/mineru/_assets/mineru_demo.yml)
...
1. Log into your Aiexec platform.
...
1. **Get your local IP address:**
   For Aiexec to correctly access the MinerU API, you need to use your LAN IP address (Do NOT use `127.0.0.1` or `localhost`). Get your IP address based on your operating system:
...
3. Configure the MinerU plugin parameters:
   ...
   **Note: Ensure that the Aiexec service can access this base URL.**
...
### 3. Important: Aiexec Environment Configuration (`FILES_URL`)

To ensure the MinerU plugin can properly handle file uploads, you need to configure the `FILES_URL` setting in Aiexec:

1. Find your Aiexec deployment directory and edit the `.env` file.
2. Modify the `FILES_URL` configuration based on your deployment method:
   ...
   - **Other deployment methods**: Set `FILES_URL` to `http://AIEXEC_HOST_IP:5001` (e.g., `http://192.168.1.101:5001`, where the IP is usually the machine running Aiexec)
3. Confirm that the Aiexec API service's port `5001` is exposed externally (check port mapping in the `docker-compose.yaml` file).
4. After saving the `.env` file, restart the Aiexec service for the configuration to take effect:
...
>   - Update the MinerU plugin's Base URL configuration in Aiexec
>   - If necessary (if Aiexec's `FILES_URL` is configured with an IP address rather than `api`), update Aiexec's `.env` file and restart the Aiexec service
> - When using the Aiexec MinerU plugin, especially when processing file uploads, if you don't configure this step, you may encounter errors like `Request URL is missing an 'http://' or 'https://' protocol`. This usually occurs because Aiexec's `api` service cannot correctly access its own file service.
