...
import (
	...
	"aiexec-plugin/cmd/commandline/plugin"
...
If no parameters are provided, an interactive mode will be started.`,
		Run: func(c *cobra.Command, args []string) {
			...
			minAiexecVersion, _ := c.Flags().GetString("min-aiexec-version")
	...
	pluginChecksumCommand = &cobra.Command{
		...
		Short: "Checksum",
		Long:  "Calculate the checksum of the plugin, you need specify the plugin path or .aiexecpkg file path",
		Args:  cobra.ExactArgs(1),
		Run: func(cmd *cobra.Command, args []string) {
			pluginPath := args[0]
...
func init() {
  ...
  - python: Python language`)
	pluginInitCommand.Flags().StringVar(&minAiexecVersion, "min-aiexec-version", "", "Minimum Aiexec version required")
