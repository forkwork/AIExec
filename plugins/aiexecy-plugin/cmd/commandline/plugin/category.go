...
const PLUGIN_GUIDE = `Before starting, here's some basic knowledge about Plugin types in Aiexec:
...
` + BOLD + `- Endpoint` + RESET + `: ` + GREEN + `Similar to Service API in Aiexec and Ingress in Kubernetes. Extend HTTP services as endpoints with custom logic.` + RESET + `
