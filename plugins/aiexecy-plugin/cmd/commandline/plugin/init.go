...
func InitPluginWithFlags(
	...
	// Set minimal Aiexec version if provided
	if minAiexecVersion != "" {
		ver := m.subMenus[SUB_MENU_KEY_VERSION_REQUIRE].(versionRequire)
		ver.SetMinimalAiexecVersion(minAiexecVersion)
...
func (m model) createPlugin() {
	...
	if m.subMenus[SUB_MENU_KEY_VERSION_REQUIRE].(versionRequire).MinimalAiexecVersion() != "" {
		manifest.Meta.MinimumAiexecVersion = parser.ToPtr(m.subMenus[SUB_MENU_KEY_VERSION_REQUIRE].(versionRequire).MinimalAiexecVersion())
	}
