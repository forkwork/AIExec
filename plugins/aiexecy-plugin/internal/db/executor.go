...
func Create(data any, ctx ...*gorm.DB) error {
	...
	return AiexecPluginDB.Create(data).Error
}
...
func Update(data any, ctx ...*gorm.DB) error {
	...
	return AiexecPluginDB.Save(data).Error
}
...
func Delete(data any, ctx ...*gorm.DB) error {
	...
	return AiexecPluginDB.Delete(data).Error
}
...
func DeleteByCondition[T any](condition T, ctx ...*gorm.DB) error {
	...
	return AiexecPluginDB.Where(condition).Delete(&model).Error
}
...
func ReplaceAssociation[T any, R any](source *T, field string, associations []R, ctx ...*gorm.DB) error {
	...
	return AiexecPluginDB.Model(source).Association(field).Replace(associations)
}
...
func AppendAssociation[T any, R any](source *T, field string, associations R, ctx ...*gorm.DB) error {
	...
	return AiexecPluginDB.Model(source).Association(field).Append(associations)
}
...
func GetAny[T any](sql string, data ...interface{}) (T /* data */, error) {
	var result T
	err := AiexecPluginDB.Raw(sql, data...).Scan(&result).Error
	return result, err
}
...
func DelAssociation[T any](field string, query ...GenericQuery) error {
	var model T
	tmp := AiexecPluginDB.Model(&model)
	for _, q := range query {
		tmp = q(tmp)
	}
...
func DropTable(model any) error {
	return AiexecPluginDB.Migrator().DropTable(model)
}

// NOTE: not used in production, only for testing
func CreateDatabase(dbname string) error {
	return AiexecPluginDB.Exec(fmt.Sprintf("CREATE DATABASE %s", dbname)).Error
}

// NOTE: not used in production, only for testing
func CreateTable(model any) error {
	return AiexecPluginDB.Migrator().CreateTable(model)
}
