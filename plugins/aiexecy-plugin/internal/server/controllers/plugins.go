...
func UploadPlugin(app *app.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		aiexecPkgFileHeader, err := c.FormFile("aiexec_pkg")
		if err != nil {
		...
		if aiexecPkgFileHeader.Size > app.MaxPluginPackageSize {
		...
		aiexecPkgFile, err := aiexecPkgFileHeader.Open()
		if err != nil {
			c.JSON(http.StatusOK, exception.BadRequestError(err).ToResponse())
			return
		}
		defer aiexecPkgFile.Close()

		c.JSON(http.StatusOK, service.UploadPluginPkg(app, c, tenantId, aiexecPkgFile, verifySignature))
	}
}

func UploadBundle(app *app.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		aiexecBundleFileHeader, err := c.FormFile("aiexec_bundle")
		if err != nil {
			c.JSON(http.StatusOK, exception.BadRequestError(err).ToResponse())
			return
		}
		...
		if aiexecBundleFileHeader.Size > app.MaxBundlePackageSize {
			c.JSON(http.StatusOK, exception.BadRequestError(errors.New("file size exceeds the maximum limit")).ToResponse())
			return
		}
		...
		aiexecBundleFile, err := aiexecBundleFileHeader.Open()
		if err != nil {
			c.JSON(http.StatusOK, exception.BadRequestError(err).ToResponse())
			return
		}
		defer aiexecBundleFile.Close()

		c.JSON(http.StatusOK, service.UploadPluginBundle(app, c, tenantId, aiexecBundleFile, verifySignature))
	}
}
