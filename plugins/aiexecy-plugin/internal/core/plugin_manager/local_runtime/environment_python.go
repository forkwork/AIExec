...
func (p *LocalPluginRuntime) InitPythonEnvironment() error {
	...
	if _, err := os.Stat(path.Join(p.State.WorkingPath, ".venv")); err == nil {
		// check if venv is valid, try to find .venv/aiexec/plugin.json
		if _, err := os.Stat(path.Join(p.State.WorkingPath, ".venv/aiexec/plugin.json")); err != nil {
			...
			// https://github.com/khulnasoft/aiexec-plugin-sdks/commit/161045b65f708d8ef0837da24440ab3872821b3b
	...
	defer func() {
		...
		if !success {
			...
			// create aiexec/plugin.json
			pluginJsonPath := path.Join(p.State.WorkingPath, ".venv/aiexec/plugin.json")
...
func (p *LocalPluginRuntime) patchPluginSdk(requirementsPath string) error {
	...
	if pluginSdkVersionObj.LessThan(version.Must(version.NewVersion("0.0.1b70"))) {
		// get aiexec-plugin path
		command := exec.Command(p.pythonInterpreterPath, "-c", "import importlib.util;print(importlib.util.find_spec('aiexec_plugin').origin)")
...
func (p *LocalPluginRuntime) getPluginSdkVersion(requirements string) (string, error) {
	...
	re := regexp.MustCompile(`(?:aiexec[_-]plugin)(?:~=|==)([0-9.a-z]+)`)
	...
	// Extract all version constraints for aiexec-plugin
	// Try to match version ranges with multiple constraints
	// For example: aiexec-plugin>=0.1.0,<0.2.0
	reAllConstraints := regexp.MustCompile(`(?:aiexec[_-]plugin)([><]=?|==)([0-9.a-z]+)(?:,([><]=?|==)([0-9.a-z]+))?`)
