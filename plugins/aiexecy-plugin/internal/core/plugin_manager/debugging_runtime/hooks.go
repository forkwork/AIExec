...
func (s *AiexecServer) OnBoot(c gnet.Engine) (action gnet.Action) {
	s.engine = c
	return gnet.None
}

func (s *AiexecServer) OnOpen(c gnet.Conn) (out []byte, action gnet.Action) {
...
func (s *AiexecServer) OnClose(c gnet.Conn, err error) (action gnet.Action) {
	// plugin disconnected
	s.pluginsLock.Lock()
...
func (s *AiexecServer) OnShutdown(c gnet.Engine) {
	close(s.shutdownChan)
}

func (s *AiexecServer) OnTraffic(c gnet.Conn) (action gnet.Action) {
...
func (s *AiexecServer) onMessage(runtime *RemotePluginRuntime, message []byte) {
	// handle message
