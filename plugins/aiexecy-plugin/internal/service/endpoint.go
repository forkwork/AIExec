...
func copyRequest(req *http.Request, hookId string, path string) (*bytes.Buffer, error) {
	...
	newReq.Header.Set("Aiexec-Hook-Id", hookId)
	// check if Aiexec-Hook-Url is set
	if url := req.Header.Get("Aiexec-Hook-Url"); url == "" {
		newReq.Header.Set(
			"Aiexec-Hook-Url",
...
func Endpoint(
	...
	// decrypt settings
	settings, err := manager.BackwardsInvocation().InvokeEncrypt(&aiexec_invocation.InvokeEncryptRequest{
...
func ListEndpoints(tenant_id string, page int, page_size int) *entities.Response {
	...
	for i, endpoint := range endpoints {
		...
		decryptedSettings, err := manager.BackwardsInvocation().InvokeEncrypt(&aiexec_invocation.InvokeEncryptRequest{
			BaseInvokeAiexecRequest: aiexec_invocation.BaseInvokeAiexecRequest{
				TenantId: tenant_id,
				UserId:   "",
				Type:     aiexec_invocation.INVOKE_TYPE_ENCRYPT,
