...
//go:embed testdata/.aiexecignore
var aiexec_ignore []byte
...
func TestPackagerAndVerifier(t *testing.T) {
	...
	// create .aiexecignore
	if err := os.WriteFile(filepath.Join(tempDir, ".aiexecignore"), aiexec_ignore, 0644); err != nil {
		t.Errorf("failed to write .aiexecignore: %s", err.Error())
		return
	}
