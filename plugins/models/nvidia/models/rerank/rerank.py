...
import requests
from aiexec_plugin.entities.model.rerank import RerankDocument, RerankResult
...
class NvidiaRerankModel(RerankModel):
    ...
    def _invoke(
        ...
        try:
            ...
            headers = {"Authorization": f"Bearer {credentials.get('api_key')}", "Accept": "application/json"}
            payload = {"model": model, "query": {"text": query}, "passages": [{"text": doc} for doc in docs]}
            ...
            for result in results["rankings"]:
                index = result["index"]
                logit = result["logit"]
                rerank_document = RerankDocument(index=index, text=docs[index], score=self._sigmoid(logit))
    ...
    def validate_credentials(self, model: str, credentials: dict) -> None:
        ...
        try:
            self._invoke(
                ...
                docs=["Example doc 1", "Example doc 2", "Example doc 3"],
            )
