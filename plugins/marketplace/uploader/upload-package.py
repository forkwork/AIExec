...
PLUGIN_DAEMON_PATH = "./aiexecy-plugin"
TESTING = False
...
def upload_directory(directory: str, token: str, base_url: str, force: bool, changelog: str):
    ...
    # delete temp.aiexecypkg if exists
    if os.path.exists("temp.aiexecypkg"):
        os.remove("temp.aiexecypkg")

    # ./aiexecy-plugin-daemon plugin package <directory> --out temp.aiexecypkg
    result = subprocess.run([PLUGIN_DAEMON_PATH, "plugin", "package", directory, "-o", "temp.aiexecypkg"], capture_output=True, text=True)
    print(result.stdout)
    print(result.stderr)
    if result.returncode != 0:
        raise Exception("Failed to package the directory")
    
    upload_package("temp.aiexecypkg", token, base_url, force, changelog)
