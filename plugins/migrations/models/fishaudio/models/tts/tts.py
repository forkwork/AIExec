...
import httpx
from aiexec_plugin.errors.model import CredentialsValidateFailedError, InvokeBadRequestError, InvokeError
from aiexec_plugin.interfaces.model.tts import TTSModel
...
class FishAudioText2SpeechModel(TTSModel):
    ...
    def get_tts_model_voices(self, model: str, credentials: dict, language: Optional[str] = None) -> list:
        ...
        params = {"self": str(not use_public_models).lower(), "page_size": "100"}
        if language is not None:
            if "-" in language:
                language = language.split("-")[0]
            params["language"] = language
        results = httpx.get(f"{api_base}/model", headers={"Authorization": f"Bearer {api_key}"}, params=params)
    ...
    def _invoke_error_mapping(self) -> dict[type[InvokeError], list[type[Exception]]]:
        ...
        """
        return {InvokeBadRequestError: [httpx.HTTPStatusError]}
