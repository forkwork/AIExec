...
class SparkLargeLanguageModel(LargeLanguageModel):
    ...
    def _generate(
        ...
        client = SparkLLMClient(model=model, **credentials_kwargs)
    ...
    def _handle_generate_stream_response(
        ...
        for index, content in enumerate(client.subscribe()):
            if isinstance(content, dict):
                ...
                delta = content
            completion += delta
            assistant_prompt_message = AssistantPromptMessage(content=delta or "")
            temp_assistant_prompt_message = AssistantPromptMessage(content=completion)
            prompt_tokens = self.get_num_tokens(model, credentials, prompt_messages)
