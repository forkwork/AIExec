...
from aiexec_plugin.entities.tool import ToolInvokeMessage, ToolParameter, ToolParameterOption
...
class AIPPTGenerateToolAdapter:
    ...
    _task = {}
    _task_type_map = {"auto": 1, "markdown": 7}
    _tool: Tool
    ...
    def _invoke(self, user_id: str, tool_parameters: dict[str, Any]) -> ToolInvokeMessage | list[ToolInvokeMessage]:
        ...
        return self._tool.create_text_message(
            f"the ppt has been created successfully,the ppt url is {ppt_url} .please give the ppt url to user and direct user to download it."
        )
    ...
    def _get_styles(cls, credentials: dict[str, str], user_id: str) -> tuple[list[dict], list[dict]]:
        ...
        if response.get("code") != 0:
            raise Exception(f"Failed to connect to aippt: {response.get('msg')}")
        colors = [
            {"id": f"id-{item.get('id')}", "name": item.get("name"), "en_name": item.get("en_name", item.get("name"))}
            for item in response.get("data", {}).get("colour") or []
        ]
        styles = [
            {"id": f"id-{item.get('id')}", "name": item.get("title")}
    ...
    def _get_suit(self, style_id: int, colour_id: int) -> int:
        ...
        headers = {
            "x-channel": "",
            "x-api-key": self._tool.runtime.credentials["aippt_access_key"],
            "x-token": self._get_api_token(credentials=self._tool.runtime.credentials, user_id="__aiexec_system__"),
        }
        response = get(
    ...
    def get_runtime_parameters(self) -> list[ToolParameter]:
        ...
        """
        try:
            (colors, styles) = self.get_styles(user_id="__aiexec_system__")
        except Exception as e:
            (colors, styles) = (
