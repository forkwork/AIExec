...
class StableDiffusionTool(Tool):
    def _invoke(
        ...
        try:
            ...
            response = post(url, json={"sd_model_checkpoint": model}, headers=headers)
    ...
    def validate_models(self):
        ...
        try:
            ...
            if response.status_code == 404:
                self._invoke(
                    user_id="test",
                    tool_parameters={"prompt": "a cat", "width": 1024, "height": 1024, "steps": 1, "lora": ""},
                )
    ...
    def img2img(
        ...
        try:
            ...
            response = post(url, json=draw_options, timeout=120, headers=headers)
            if response.status_code != 200:
