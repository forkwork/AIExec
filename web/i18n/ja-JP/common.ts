const translation = {
  ...
  account: {
    ...
    khulnaSoftAccount: 'Aiexecアカウント',
    khulnaSoftAccountTip: 'Aiexecアカウントと関連するユーザーデータ。',
    editName: '名前を編集',
  ...
  members: {
    ...
    invitationSentTip: '招待が送信され、彼らはAiexecにサインインしてあなた様のチームデータにアクセスできます。',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'Aiexecが提供するOpenAIホスティングサービスを使用すると、GPT-3.5などのモデルを使用できます。トライアルクォータが使い果たされる前に、他のモデルプロバイダを設定する必要があります。',
      callTimes: '通話回数',
  ...
  apiBasedExtension: {
    title: 'API拡張機能は、Aiexecのアプリケーション全体での簡単な使用のための設定を簡素化し、集中的なAPI管理を提供します。',
