...
const ModelIcon: FC<ModelIconProps> = ({
  ...
  if (provider?.provider && ['openai', 'khulnasoft/openai/openai'].includes(provider.provider) && modelName?.includes('gpt-4o'))
    return <div className='flex items-center justify-center'><OpenaiBlue className={cn('h-5 w-5', className)} /></div>
  if (provider?.provider && ['openai', 'khulnasoft/openai/openai'].includes(provider.provider) && modelName?.startsWith('gpt-4'))
