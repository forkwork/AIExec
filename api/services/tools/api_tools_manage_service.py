        ...
        try:
            ...
            credentials_schema = [
                ProviderConfig(
                    ...
                    default="none",
                    options=[
                        ProviderConfig.Option(value="none", label=I18nObject(en_US="None", zh_Hans="无", bn_BD="কোনটিই নয়")),
                        ProviderConfig.Option(value="api_key", label=I18nObject(en_US="Api Key", zh_Hans="Api Key")),
                    ],
                    placeholder=I18nObject(en_US="Select auth type", zh_Hans="选择认证方式", bn_BD="প্রমাণীকরণ পদ্ধতি নির্বাচন করুন"),
                ),
                ProviderConfig(
                    name="api_key_header",
                    type=ProviderConfig.Type.TEXT_INPUT,
                    required=False,
                    placeholder=I18nObject(en_US="Enter api key header", zh_Hans="输入 api key header，如：X-API-KEY", bn_BD="API কী হেডার লিখুন, উদাহরণস্বরূপ: X-API-KEY"),
                    default="api_key",
                    help=I18nObject(en_US="HTTP header name for api key", zh_Hans="HTTP 头部字段名，用于传递 api key", bn_BD="API কী প্রেরণের জন্য HTTP হেডার ফিল্ডের নাম"),
                ),
                ProviderConfig(
                    name="api_key_value",
                    type=ProviderConfig.Type.TEXT_INPUT,
                    required=False,
                    placeholder=I18nObject(en_US="Enter api key", zh_Hans="输入 api key", bn_BD="API কী লিখুন"),
                    default="",
                ),
            ]
