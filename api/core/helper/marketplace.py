...
def get_plugin_pkg_url(plugin_unique_identifier: str):
    return (URL(str(aiexec_config.MARKETPLACE_API_URL)) / "api/v1/plugins/download").with_query(
...
def download_plugin_pkg(plugin_unique_identifier: str):
    ...
    return download_with_size_limit(url, aiexec_config.PLUGIN_MAX_PACKAGE_SIZE)
...
def batch_fetch_plugin_manifests(plugin_ids: list[str]) -> Sequence[MarketplacePluginDeclaration]:
    ...
    url = str(URL(str(aiexec_config.MARKETPLACE_API_URL)) / "api/v1/plugins/batch")
...
def record_install_plugin_event(plugin_unique_identifier: str):
    url = str(URL(str(aiexec_config.MARKETPLACE_API_URL)) / "api/v1/stats/plugins/install_count")
