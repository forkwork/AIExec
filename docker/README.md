...
Welcome to the new `docker` directory for deploying Aiexec using Docker Compose. This README outlines the updates, deployment instructions, and migration details for existing users.
...
### How to Deploy Aiexec with `docker-compose.yaml`
...
### How to Deploy Middleware for Developing Aiexec
...
2. **Running Middleware Services**:
    - Navigate to the `docker` directory.
    - Execute `docker compose -f docker-compose.middleware.yaml --profile weaviate -p aiexec up -d` to start the middleware services. (Change the profile to other vector database if you are not using weaviate)
