...
from aiexec_plugin.config.config import AiexecPluginEnv, InstallMethod
from aiexec_plugin.config.logger_format import plugin_logger_handler
from aiexec_plugin.core.entities.message import InitializeMessage
from aiexec_plugin.core.entities.plugin.request import (
    AgentActions,
    EndpointActions,
    ModelActions,
    PluginInvokeType,
...
from aiexec_plugin.core.plugin_executor import PluginExecutor
from aiexec_plugin.core.plugin_registration import PluginRegistration
from aiexec_plugin.core.runtime import Session
...
from aiexec_plugin.core.server.io_server import IOServer
from aiexec_plugin.core.server.router import Router
from aiexec_plugin.core.server.serverless.request_reader import ServerlessRequestReader
from aiexec_plugin.core.server.stdio.request_reader import StdioRequestReader
from aiexec_plugin.core.server.stdio.response_writer import StdioResponseWriter
...
class Plugin(IOServer, Router):
    def __init__(self, config: AiexecPluginEnv) -> None:
        """
        Initialize plugin
        """
    ...
    def _launch_local_stream(self, config: AiexecPluginEnv) -> tuple[RequestReader, Optional[ResponseWriter]]:
        """
        Launch local stream
        """
    ...
    def _launch_remote_stream(self, config: AiexecPluginEnv) -> tuple[RequestReader, Optional[ResponseWriter]]:
        """
    ...
    def _launch_serverless_stream(self, config: AiexecPluginEnv) -> tuple[RequestReader, Optional[ResponseWriter]]:
        """
    ...
    def _execute_request(
        ...
        session = Session(
            ...
            aiexec_plugin_daemon_url=self.config.AIEXEC_PLUGIN_DAEMON_URL,
    ...
    @staticmethod
    def _get_remote_install_host_and_port(config: AiexecPluginEnv) -> tuple[str, int]:
        """
        Get host and port for remote installation
        :param config: Aiexec plugin env config
