...
from aiexec_plugin.entities.tool import LogMetadata, ToolInvokeMessage, ToolParameter, ToolRuntime, ToolSelector
from aiexec_plugin.file.constants import AIEXEC_FILE_IDENTITY, AIEXEC_TOOL_SELECTOR_IDENTITY
from aiexec_plugin.file.entities import FileType
from aiexec_plugin.file.file import File
...
class ToolLike(ABC, Generic[T]):
    ...
    def _convert_parameters(cls, tool_parameters: dict) -> dict:
        ...
        for parameter, value in tool_parameters.items():
            if isinstance(value, dict) and value.get("aiexec_model_identity") == AIEXEC_FILE_IDENTITY:
                ...
                isinstance(item, dict) and item.get("aiexec_model_identity") == AIEXEC_FILE_IDENTITY for item in value
            ):
            ...
            elif isinstance(value, dict) and value.get("aiexec_model_identity") == AIEXEC_TOOL_SELECTOR_IDENTITY:
                tool_parameters[parameter] = ToolSelector.model_validate(value)
            elif isinstance(value, list) and all(
                isinstance(item, dict) and item.get("aiexec_model_identity") == AIEXEC_TOOL_SELECTOR_IDENTITY
                for item in value
            ):
