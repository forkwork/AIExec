...
from aiexec_plugin.config.config import InstallMethod
from aiexec_plugin.core.entities.invocation import InvokeType
from aiexec_plugin.core.entities.plugin.io import (
    PluginInStream,
    PluginInStreamBase,
    PluginInStreamEvent,
)
from aiexec_plugin.core.server.__base.request_reader import RequestReader
from aiexec_plugin.core.server.__base.response_writer import ResponseWriter
from aiexec_plugin.core.server.tcp.request_reader import TCPReaderWriter
...
class ModelInvocations:
    def __init__(self, session: "Session") -> None:
        from aiexec_plugin.invocations.model.llm import LLMInvocation, SummaryInvocation
        from aiexec_plugin.invocations.model.moderation import ModerationInvocation
        from aiexec_plugin.invocations.model.rerank import RerankInvocation
        from aiexec_plugin.invocations.model.speech2text import Speech2TextInvocation
        from aiexec_plugin.invocations.model.text_embedding import TextEmbeddingInvocation
        from aiexec_plugin.invocations.model.tts import TTSInvocation
...
class AppInvocations:
    def __init__(self, session: "Session"):
        from aiexec_plugin.invocations.app import FetchAppInvocation
        from aiexec_plugin.invocations.app.chat import ChatAppInvocation
        from aiexec_plugin.invocations.app.completion import CompletionAppInvocation
        from aiexec_plugin.invocations.app.workflow import WorkflowAppInvocation
...
class WorkflowNodeInvocations:
    def __init__(self, session: "Session"):
        from aiexec_plugin.invocations.workflow_node.parameter_extractor import (
            ParameterExtractorNodeInvocation,
        )
        from aiexec_plugin.invocations.workflow_node.question_classifier import (
...
class Session:
    def __init__(
        ...
        aiexec_plugin_daemon_url: Optional[str] = None,
        ...
        # aiexec plugin daemon url
        self.aiexec_plugin_daemon_url: Optional[str] = aiexec_plugin_daemon_url
    ...
    def _register_invocations(self) -> None:
        ...
        from aiexec_plugin.invocations.storage import StorageInvocation
        from aiexec_plugin.invocations.tool import ToolInvocation
...
class BackwardsInvocation(Generic[T], ABC):
    ...
    def _backwards_invoke(
        ...
        """
        backwards invoke aiexec depends on current runtime type
        """
    ...
    def _http_backwards_invoke(
        ...
        """
        if not self.session or not self.session.aiexec_plugin_daemon_url:
            raise Exception("current tool runtime does not support backwards invoke")

        url = URL(self.session.aiexec_plugin_daemon_url) / "backwards-invocation" / "transaction"
        headers = {
            "Aiexec-Plugin-Session-ID": self.session.session_id,
        }
