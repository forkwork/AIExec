...
func TestApi3(t *testing.T) {
	var c = &aiexec.ClientConfig{
		Host:         host,
		ApiSecretKey: apiSecretKey,
	}
	var client = aiexec.NewClientWithConfig(c)

	ctx := context.Background()

	var (
		ch  = make(chan aiexec.ChatMessageStreamChannelResponse)
		err error
	)

	ch, err = client.Api().ChatMessagesStream(ctx, &aiexec.ChatMessageRequest{
		Query: "你是谁?",
		User:  "这里换成你创建的",
	})
	if err != nil {
...
func TestMessages(t *testing.T) {
	...
	// messages
	var messageReq = &aiexec.MessagesRequest{
		ConversationID: cId,
		User:           "jiuquan AI",
	}

	var client = aiexec.NewClient(host, apiSecretKey)

	var msg *aiexec.MessagesResponse
...
func TestMessagesFeedbacks(t *testing.T) {
	...
	var res *aiexec.MessagesFeedbacksResponse
	if res, err = client.Api().MessagesFeedbacks(ctx, &aiexec.MessagesFeedbacksRequest{
		MessageID: id,
		Rating:    aiexec.FeedbackLike,
		User:      "jiuquan AI",
	}); err != nil {
		t.Fatal(err.Error())
	}
...
func TestConversations(t *testing.T) {
	...
	var res *aiexec.ConversationsResponse
	if res, err = client.Api().Conversations(ctx, &aiexec.ConversationsRequest{
		User: "jiuquan AI",
	}); err != nil {
		t.Fatal(err.Error())
	}
...
func TestConversationsRename(t *testing.T) {
	...
	var res *aiexec.ConversationsRenamingResponse
	if res, err = client.Api().ConversationsRenaming(ctx, &aiexec.ConversationsRenamingRequest{
...
func TestParameters(t *testing.T) {
	...
	var res *aiexec.ParametersResponse
	if res, err = client.Api().Parameters(ctx, &aiexec.ParametersRequest{
		User: "jiuquan AI",
	}); err != nil {
		t.Fatal(err.Error())
	}
...
func TestRunWorkflow(t *testing.T) {
	client := aiexec.NewClient(host, apiSecretKey)
	//client := aiexec.NewClient("https://aiexec.zhaokm.org", "app-")

	// 测试带图片的工作流请求
	workflowReq := aiexec.WorkflowRequest{
...
func TestRunWorkflowStreaming(t *testing.T) {
	...
	handler := &testEventHandler{
		t:  t,
		mu: &mu,
		onStreamingResponse: func(resp aiexec.StreamingResponse) {
			mu.Lock()
			defer mu.Unlock()

			switch resp.Event {
			case aiexec.EventWorkflowStarted:
				workflowStarted = true
			case aiexec.EventNodeStarted:
				nodeStarted = true
			case aiexec.EventNodeFinished:
				nodeFinished = true
				if resp.Data.ExecutionMetadata.TotalTokens > 0 {
					t.Logf("Node used %d tokens", resp.Data.ExecutionMetadata.TotalTokens)
				}
			case aiexec.EventWorkflowFinished:
		...
		onTTSMessage: func(msg aiexec.TTSMessage) {
			mu.Lock()
			defer mu.Unlock()
...
type testEventHandler struct {
	t                   *testing.T
	mu                  *sync.Mutex
	onStreamingResponse func(aiexec.StreamingResponse)
	onTTSMessage        func(aiexec.TTSMessage)
}

func (h *testEventHandler) HandleStreamingResponse(resp aiexec.StreamingResponse) {
	if h.onStreamingResponse != nil {
		h.onStreamingResponse(resp)
	}
}

func (h *testEventHandler) HandleTTSMessage(msg aiexec.TTSMessage) {
